...
label class_room_part_1:
    ...
    # 소개할까?
    window show "어떻게 소개할까..."

    menu:
        "자신있게 소개하자!":
            window show 
            p "안녕! 내 이름은 [p]야. 잘 부탁해!"
            t "좋아. 다들 잘 지내보도록 해."
            "선생님은 분위기를 가볍게 넘기며 조용히 교실을 둘러보았다."
...
#     de "{size=25}전학 첫 날에 바로 잔다니.. 너도 대단하네{/size}"
#     de "{size=25}이제 점심시간이라 밥 먹자고 깨웠어.{/size}"
#     de "{size=25}반가워. 난 태구라고 해.{/size}"
#     p "{size=25}뭐지...? 저 슬라임은...{/size}"
#     "{size=25}[p] (은)는 순간 당황했지만, 곧 고개를 살짝 끄덕였다.{/size}"
#     p "{size=25}아... 어. 반가워.{/size}"
#     de "{size=25}내가 관상을 좀 볼 줄 알거든.{/size}"
#     de "{size=25}너 애니메이션 좋아해?{/size}"
#     de "{size=25}내가 요즘 만화를 그리고 있는데{/size}"
#     "{size=25}태구는 핸드폰을 꺼내 화면을 보여주었다.{/size}"
#     de "{cps=60}여기 만화 주인공이 나인데 여자한테 관심 없는 약간 <미소년 부잣집 도련님> 느낌이랄까나?{/size}"
#     de "{cps=60}근데 이제 <여자 메이드>가 나를 좋아해가지고 근데 이제 내가 좀 귀찮아하는데 계속 나한테 애원하는거야.{/size}"
#     de "{cps=60}그렇지만 나는!{/size}"
#     de "{cps=60}그런 거에 신경쓰지 않고 이 세계를 구하기 위한 <용사> 역할이어서 열심히 해야하는 부분인데{/size}"
#     de "{cps=60}어쨌든 내가 위험에 빠지자 메이드가 나를 구해주는 내용이야{/size}"
#     de "{size=25}알겠지?{/size}"
#     "{size=25}[p](은)는 다시 한 번 당황했지만, 태구는 대답을 기다리지도 않고 말을 이어갔다.{/size}"
#     de "{cps=60}그래서 내가 메이드에게 너의 정체가 뭐냐고 물어볼텐데 여기서 진짜 중요한 대사가 나오거든?{/size}"
#     de "{cps=60}잊으셨어요.. 용사님? 저에요. 라고 메이드가 대답하는거야.{/size}"
#     p "{size=25}...{/size}"
#     de "{size=25}아, 이게 아닌가? 혹시 애니 음악을 좋아해?{/size}"
#     de "{size=25}나 애니 오프닝 엄청 좋아하거든.{/size}"
#     "{size=25}다시 태구는 핸드폰을 꺼내 화면을 보여주었다.{/size}"
#     de "{cps=60}혹시 <비스크돌> 오프닝 알아?{/size}"
#     p "{size=25}...{/size}"
#     de "{cps=60}그러면, <소드 아트 온라인> 오프닝 알아?{/size}"
#     p "{size=25}...{/size}"
#     de "{cps=60}그러면... <방패 용사 성공담> 오프닝 알아?{/size}"
#     p "{size=25}...모르겠어{/size}"
#     de "{cps=60}그러면... <청춘돼지는 바니걸 선배의 꿈을 꾸지 않는다> 오프닝 알아?{/size}"
#     p "{size=25}아니... 몰라..{/size}"
#     de "{cps=60}그러면! <최애의 아이> 오프닝 알아?{/size}"
#     p "{size=25}그건.. 알아..{/size}"
#     de "{cps=60}그러면...! <B코마치> 알아?{/size}"
#     p "{size=25}그건... 몰라...{/size}"
#     de "{size=25}이 노래 진짜 좋아. 너도 한 번 들어봐. 딱 내 스타일이야.{/size}"
...
#     de "{size=25}...{/size}"
#     de "{size=25}그러니까 좋아하는 애니는 있는거지?{/size}"
...
label yuna_leave_1:
    ...
    de "{size=25}놀랐지? 원래 저런 애야.{/size}"
    "{size=25}[p](은)는 천천히 고개를 돌려 태구를 보았다. 짧게 한숨을 내쉬며 고개를 끄덕였다.{/size}"
    show daeun_1 at center with fade
    play sound "audio/bgm/taegu.mp3"
    de "{size=25}방금 너한테 말 걸었던 애는 {gold}예나{/gold}야. 우리 학교에서 모르는 애 없을걸?{/size}"
    de "{size=25}{gold}밴드부 리더{/gold}인데, 진짜 실력도 대단하고.{/size}"
    de "{cps=10}...그리고... 예뻐.{/size}"
    "{size=25}[p](은)는 천천히 시선을 창밖으로 돌렸다. 태구는 그런 [p]를 보며 말을 더 이어갔다.{/size}"

    de "{size=25}그런데 너한테 관심 가지는 거 보니까 신기하네. 보통 저렇게 들이대는 스타일은 아닌데 말이야.{/size}"
    "{size=25}[p](은)는 태구의 말을 듣고도 별다른 반응이 없었다. 그는 무언가 말하려다 이내 고개를 숙였다.{/size}"
    p "{size=25}...그냥 별 일 아닐거야.{/size}"
    de "{size=25}그래도 이상하단 말이지.. 아까 들어보니까 유나가 락 페스티벌 얘기하던데? 진짜 너 기타 쳤던 거 아니야?{/size}"
    p "{size=25}...그냥 착각했을거야.{/size}"
    "{size=25}[p](은)는 흠칫하며 태구를 보다가 시선을 피하며 대답했다.{/size}"
    de "{size=25}진짜? 근데, 걔가 그런 그런 착각을 할 애가 아닌데. 특히 음악 쪽으로는 엄청 까다롭거든.{/size}"
    p "{size=25}...{/size}"
    de "{size=25}음... 그래도 유나가 너한테 관심 가졌다는 거 하나만큼은 확실한데. 조심해.{/size}"
    de "{size=25}한 번 찍히면 놓는 법이 없는 애니까.{/size}"
    p "{size=25}...{/size}"
