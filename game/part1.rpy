...
label class_room_part_1:
    ...
    t "{cps=20}자, [p]아 자기 소개해볼래?{/cps}"

    # 소개할까?
    menu:
        "자신있게 소개하자!":
            p "{cps=20}안녕! 내 이름은 [p]야. 잘 부탁해!{/cps}"
            t "{cps=20}좋아. 다들 잘 지내보도록 해.{/cps}"
...
label taegu_part_1:
    ...
    p "{blue}{cps=20}뭐지...? 저 슬라임은...{/cps}{/blue}"
    ...
    p "{cps=20}그건... 몰라...{/cps}"
    tg "{cps=20}이 노래 진짜 좋아. 너도 한 번 들어봐. 딱 내 스타일이야.{/cps}"
    "{blue}{cps=20}[p](은)는 태구가 보여준 핸드폰 화면을 흘깃 보며 피식 웃었다.{/cps}{/blue}"
    "{blue}{cps=20}태구의 열정이 재미있게 느껴지기 시작했다.{/cps}{/blue}"
    ...
    pause 2
    p "{blue}{cps=20}(근데 음악을 다시 하진 않을 거야..){/cps}{/blue}"
    scene black
    scene bg school_classroom with fade
    pause 1.5
    ...
    tg "{cps=20}...{/cps}"
    tg "{cps=20}그러니까 좋아하는 애니는 있는거지?{/cps}"

    p "{cps=10}.....{/cps}"
    "{blue}{cps=20}[p](은)는 다시 책상에 팔을 베고 엎드려 눈을 감았다.{/cps}{/blue}"
...
label yuna_part_1:
    scene black with fade
    pause 0.5
    y "{cps=20}그 전학생이 왔다고 들었는데?{/cps}"
    y "{cps=20}여기 전학생이 전학 온 반 맞지?!{/cps}"
    window hide
    scene black with flash
    pause 1
    scene black with flash
    ...
    "{blue}{cps=20}[p](은)는 생기발랄한 목소리에 화들짝 눈을 떴다.{/cps}{/blue}"
    "{blue}{cps=20}시야가 또렷해지기도 전에, 반짝이는 눈동자가 가까이 다가왔다.{/cps}{/blue}"
    "{blue}{cps=20}그녀는 [p]의 책상 옆에 몸을 기울이며, 해맑은 미소를 띤 채 [p](을)를 내려다보았다.{/cps}{/blue}"

    show bg yena_appear_2 with fade

    y "{cps=20}너 맞지? 저번 락 페스티벌에서 상 받았던 애!{/cps}"
    "{blue}{cps=20}[p](은)는 그녀의 말에 당황하며 얼굴을 빨개졌다.{/cps}{/blue}"
    p "{cps=20}...락 페스티벌? {/cps}"
    y "{cps=20}내가 그때 바로 앞에서 봤거든. 진짜 멋있었어. 그 솔로 연주! 너잖아?{/cps}"
    "{blue}{cps=20}발랄한 그녀는 미소를 띠며 눈을 바라보았다.{/cps}{/blue}"
    "{blue}{cps=20}[p](은)는 말을 잇지 못하고 고개를 피하며 어색하게 대답했다.{/cps}{/blue}"

    p "{cps=20}...아니야. 사람 잘못 본 거야.{/cps}"

    window hide
    show bg school_classroom with fade
    show yena_1 at center with dissolve

    y "{cps=20}뭐야? 내가 딱 봤는데? 네 얼굴이 똑같았어.{/cps}"
    y "{cps=20}그리고 그 기타 연주... 아무나 할 수 있는 게 아니던데.{/cps}"
    y "{cps=20}그러니까 너 맞지?{/cps}"

    p "{cps=20}...{/cps}"
    p "{cps=20}이제 그런 건 옛날 얘기야. 지금은 기타 치지도 않고, 앞ㅡ로도 안 칠거야...{/cps}"

    y "{cps=20}왜? 그런 재능을 왜 묻어둬? 그냥 취미로라도 하면 되잖아.{/cps}"
    p "{cps=20}...관심없어.{/cps}"

    "{blue}{cps=20}그녀는 [p]의 태도에 당황하기는커녕, 오히려 더 흥미로워진 듯 [p]에게 다가섰다.{/cps}{/blue}"
    hide yena_1 with dissolve
    show yena_2 at center with dissolve
    y "{cps=20}이상하네..? 그런 표정으로 관심 없다고 말하면 누가 믿어?{/cps}"
    y "{cps=20}너 사실 다시 음악하고 싶어 하는 거 아니야?{/cps}"

    "{cps=20}[p](은)는 그녀의 말에 움찔하며 눈길을 피했다. 왠지 그녀의 말이 자신을 꿰뚫어 본 것처럼 느껴졌다.{/cps}"
    y "{cps=20}흥, 너 거짓말은 못하겠다~{/cps}"
    y "{cps=20}좋아. 이렇게 하자. 내가 오늘 방과후에 음악실에서 기다릴게.{/cps}"
    y "{cps=20}네가 와서 한 곡만 연주해주면 넘어가줄게!{/cps}"
    p "{cps=20}그럴 리 없잖아...{/cps}"
    y "{cps=20}진짜라니까. 내가 이래뵈도 약속 잘 지키는 사람이라구.{/cps}"
    y "{cps=20}그럼 약속이야! 방과후에 음악실에서 만나자!{/cps}"

    hide yena_2 with dissolve
    stop music fadeout 1
    stop sound fadeout 1

    "{blue}{cps=20}그녀는 말을 끝내고 미소를 띠며 교실을 나갔다.{/cps}{/blue}"
    jump yuna_leave_1
...
label yuna_leave_1:
    ...
    window hide
    scene bg school_classroom with fade
    "{blue}{cps=20}한바탕 소란이 지나고, [p](은)는 다시 창 밖을 바라보며 짧게 한숨을 내쉬었다.{/cps}{/blue}"
    p "{cps=20}...락 페스티벌이라니. 그걸 알아본 애가 있을 줄이야.{/cps}"
    ...
    play sound "audio/se/camera 1.wav"
    show bg memory_3 with flash
    "{blue}{cps=20}그는 잠시 그 날의 기억에 잠겼다.{/cps}{/blue}"
    "{blue}{cps=20}수많은 관객들 앞에서 연주하던 순간과, 그 뒤를 따라온 상처가 머릿속을 스친다. 그러나 그는 고개를 저으며 다시 그 기억을 밀어냈다.{/cps}{/blue}"
    scene bg school_classroom with fade
    p "{cps=20}지금은 아무 의미 없는데...{/cps}"
    tg "{cps=20}놀랐지? 원래 저런 애야.{/cps}"
    "{blue}{cps=20}[p](은)는 천천히 고개를 돌려 태구를 보았다. 짧게 한숨을 내쉬며 고개를 끄덕였다.{/cps}{/blue}"
    show taegu_1 at center with fade
    play sound "audio/bgm/taegu.mp3"
    tg "{cps=20}방금 너한테 말 걸었던 애는 {gold}예나{/gold}야. 우리 학교에서 모르는 애 없을걸?{/cps}"
    tg "{cps=20}{gold}밴드부 리더{/gold}인데, 진짜 실력도 대단하고.{/cps}"
    tg "{cps=10}...그리고... 예뻐.{/cps}"
    "{blue}{cps=20}[p](은)는 천천히 시선을 창밖으로 돌렸다. 태구는 그런 [p]를 보며 말을 더 이어갔다.{/cps}{/blue}"

    tg "{cps=20}그런데 너한테 관심 가지는 거 보니까 신기하네. 보통 저렇게 들이대는 스타일은 아닌데 말이야.{/cps}"
    "{blue}{cps=20}[p](은)는 태구의 말을 듣고도 별다른 반응이 없었다. 그는 무언가 말하려다 이내 고개를 숙였다.{/cps}{/blue}"
    p "{cps=20}...그냥 별 일 아닐거야.{/cps}"
    tg "{cps=20}그래도 이상하단 말이지.. 아까 들어보니까 유나가 락 페스티벌 얘기하던데? 진짜 너 기타 쳤던 거 아니야?{/cps}"
    p "{cps=20}...그냥 착각했을거야.{/cps}"
    "{blue}{cps=20}[p](은)는 흠칫하며 태구를 보다가 시선을 피하며 대답했다.{/cps}{/blue}"
    tg "{cps=20}진짜? 근데, 걔가 그런 그런 착각을 할 애가 아닌데. 특히 음악 쪽으로는 엄청 까다롭거든.{/cps}"
    p "{cps=20}...{/cps}"
    tg "{cps=20}음... 그래도 유나가 너한테 관심 가졌다는 거 하나만큼은 확실한데. 조심해.{/cps}"
    tg "{cps=20}한 번 찍히면 놓는 법이 없는 애니까.{/cps}"
    p "{cps=20}...{/cps}"
